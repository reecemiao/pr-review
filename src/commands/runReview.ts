...
import { getBaseBranch, getMaxAgentIterations, getModelSelector, getToolScope } from '../config/settings';
...
export function registerRunReview(context: vscode.ExtensionContext): vscode.Disposable[] {
    ...
    const runFromPr = vscode.commands.registerCommand('prReview.runFromPrList', async (node: unknown) => {
...
async function runReview(context: vscode.ExtensionContext): Promise<void> {
    ...
    await vscode.window.withProgress(
        ...
        async (progress, token) => {
            ...
            const template = await loadTemplate(context.extensionUri, folder.uri, diffResult.changedFiles);
            ...
            const pr = repo ? await findOpenPr(repo.owner, repo.name, headBranch).catch(() => null) : null;

            progress.report({ message: 'Running review agent…' });
            const tools = getToolsForScope(getToolScope());
            const userPrompt = buildUserPrompt(diffResult.diff, diffResult.changedFiles, template.languages);
            ...
            ReviewPanel.create(context.extensionUri, result, {
                ...
                onCopyMarkdown: (findings, decision, summary) => copyAsMarkdown(findings, decision, summary),
                onSubmit: async (payload) => {
                    if (!repo || !pr) {
                        return { ok: false, error: 'No open PR for this branch.' };
                    }
                    try {
                        const selected = agentResult.findings.filter((f) => payload.selectedIds.includes(f.id));
...
async function copyAsMarkdown(findings: Finding[], decision: ReviewDecision, summary: string): Promise<void> {
